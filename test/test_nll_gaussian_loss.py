...
class TestDirichletLoss(unittest.TestCase):
    ...
    def test_gaussian_nll_two_dim(self):
        target = torch.unsqueeze(torch.tensor([[1.0, 2.0]], dtype=torch.float), dim=1)
        mean = torch.unsqueeze(torch.tensor([[0.5, 1.0]], dtype=torch.float), dim=1)
        var = torch.unsqueeze(torch.tensor([[2.0, 5.0]], dtype=torch.float), dim=1)
    ...
    def test_gaussian_nll_batch(self):
        target = torch.unsqueeze(torch.tensor([[1.0], [0.75]], dtype=torch.float), dim=-1)
        mean = torch.unsqueeze(torch.tensor([[0.5], [0.25]], dtype=torch.float), dim=-1)
        var = torch.unsqueeze(torch.tensor([[10.0], [5.0]], dtype=torch.float), dim=-1)
