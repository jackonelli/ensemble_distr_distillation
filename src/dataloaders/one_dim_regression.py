...
class SyntheticRegressionData(torch.utils.data.Dataset):
        ...
        with self.file.open(newline="") as csv_file:
            ...
            for count, row in enumerate(csv_reader):
                if count == index:
                    ...
                    break
        inputs = sample[:-1]
        targets = int(float(sample[-1]))
        return (np.array(inputs,
                         dtype=np.float32), np.array(targets, dtype=np.long))
    ...
    def sample_new_data(self):
        ...
        if self.train:
            ...
            all_x = np.linspace(-5, 5, self.n_samples)

        all_y = 7 * np.sin(all_x) + 3 * np.abs(np.cos(all_x)) * np.random.normal(size=[self.n_samples, 1])
