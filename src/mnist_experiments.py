    ...
    train_loader = torch.utils.data.DataLoader(
        torchvision.datasets.MNIST('/files/', train=True, download=True,
                                   transform=torchvision.transforms.Compose([
                                       ...
                                       utils.ReshapeTransform((-1,))])),  # torchvision.transforms.Normalize((0.1307,), (0.3081,)
...
def rotation_entropy(img, angles, model):
    ...
    data_set_flattened = data_set.view(data_set.shape[0], data_set.shape[1] * data_set.shape[2])
...
def main():
    ...
    for i in range(num_ensemble_members):
        ...
        model = models.NeuralNet(input_size, hidden_size_1, hidden_size_2, output_size, lr=args.lr)
        model.train(train_loader, args.num_epochs)
        print("Accuracy on test data: {}".format(get_accuracy(model, test_loader)))
        prob_ensemble.add_member(model)

    print("Ensemble accuracy on test data {}".format(get_accuracy(prob_ensemble, test_loader)))

    distilled_model = distilled_network.PlainProbabilityDistribution(
        input_size, hidden_size_1, hidden_size_2, output_size, prob_ensemble, lr=0.001)

    print("Training distilled network")
    distilled_model.train(train_loader, args.num_epochs * 2, t=1)
    print("Distilled model accuracy on test data: {}".format(get_accuracy(distilled_model, test_loader)))
    ...
    distilled_model_entropy, distilled_model_rotation_prediction = rotation_entropy(test_img, angles, distilled_model)
